...
**Very important**: A utility function is not unique. If $u$ represents $\succeq$, then for any strictly increasing function $f$, the function $f \circ u$ also represents $\succeq$. This is because if $u(A) \geq u(B)$, then $f(u(A)) \geq f(u(B))$ since $f$ is strictly increasing.
...
-   Strictly monotonic preferences $\iff$ strictly increasing utility functions
    -   Preferences: $x \geq y \text{ and } x \neq y \implies x \succ y$
    -   Utility: $x \geq y \text{ and } x \neq y \implies u(x) > u(y)$
...
Concavity implies quasiconcavity (proof as exercise). Quasiconcavity is preserved under a strictly increasing transformation, which makes it a desirable property of utility functions since representation is not unique. To check quasiconcavity, it is often easier to check the quasiconcavity of its transformation. If a strictly increasing transformation of a utility function is concave, then the orginal utility function is quasiconcave (not necessarily concave), why?

Quasiconcavity does not imply concavity. For example, consider the Cobb-Douglas utility function $u=x^a y^b$, where $a,b>0$. Suppose $a=b=1/4$, then $u$ is strictly concave, hence is also quasiconcave. For $a=b=3/2$, the utility function is quasiconcave but not concave.
...
$$
\begin{aligned}
& L_i(x^*) \equiv \frac{\partial L}{\partial x_i}(x^*) = 0, \quad  i=1,\dots, L \\ 
% & L_i(x^*) = 0 \text{ if } x^*_i>0, \quad i=1,\dots,L \\
& \lambda^* \geq 0, \: w - px^* \geq 0 \text{ and } \lambda^*(w - px^*) = 0. 
\end{aligned}
$$ {#eq-NFOCs}

The first set of conditions yields 
$$
u_i(x^*) = \lambda^* p_i.
$$
...
-   $\lambda^*$ is often interpreted as the shadow price: the increase in the maximized objective function (indirect utility in this context) as the constraint is relaxed by one unit

-   In other words, it is the shadow price on the constraint (on $w$)

-   Mathematically: 
    $$
    \lambda^* = \frac{\partial u(x^*(p,w))}{\partial w} \equiv \frac{\partial v}{\partial w},
    $$

where $v(p,w) = u(x^*(p,w))$ is called the indirect utility function. 
...
The second line comes from the first-order conditions @eq-nec_cond_UMP.
And the third line comes from the fact the budget constraint binds at the optimum, which implies:
$$
...
### Example: Cobb-Douglas utility function with availability constraints

Let us consider the Cobb-Douglas utility function $u(x_1, x_2) = x_1^{\alpha}x_2^{1-\alpha}$ with $\alpha\in(0,1)$. The consumer faces the budget constraint $p_1x_1 + p_2x_2 \leq w$ (Case 1) and an availability constraint on one of the goods $x_1 \leq \bar{x}_1$ or $x_2 \leq \bar{x}_2$ (Case 2). Play around with the interactive below to see how the optimal bundle changes as we vary the parameters of the problem.

<iframe id="kt-frame" src="interactives/kuhn-tucker-utility.html" style="width:100%; border:none;" scrolling="no"></iframe>
<script>
  const ktFrame = document.getElementById('kt-frame');
  function resizeKtFrame(){ try { ktFrame.style.height = ktFrame.contentWindow.document.body.scrollHeight + 40 + 'px'; } catch(e){} }
  ktFrame.addEventListener('load', resizeKtFrame);
  window.addEventListener('resize', resizeKtFrame);
  setTimeout(resizeKtFrame, 1200);
</script>
...
One way to explain this paradoxical behavior is to model preferences using a bounded utility function. The individual is then willing to pay up to the certainty equivalent of the game given her utility function. For a concave utility function, which captures risk aversion, the certainty equivalent is smaller than the expected value of the game. In particular, it can be shown that if the utility function is logarithmic, the certainty equivalent is 4.
