...
-   $C_i$ plant $i$th's cost of production
-   $R_i = p(Q)q_i$ the total revenu of plant $i$th

The PMP is: 

$$
\begin{aligned}
\max_{q_1,q_2} \pi_1 + \pi_2 &= R_1(q_1, q_2) - C_1(q_1) + R_2(q_1, q_2) - C_2(q_2) \\
&= p(q_1, q_2)q_1 - C_1(q_1) + p(q_1, q_2)q_2 - C_2(q_2) \\
&= p(q_1, q_2)(q_1 + q_2) - C_1(q_1) - C_2(q_2).
\end{aligned}
$${#eq-PMP_multi}
...
One public solution is to allow natural monopolists to charge different prices to different groups of customers (e.g., different electricity/water rates depending on whether customers are businesses or households) in such a way that the losses incurred would be offset by the positive profits. This approach follows the so-called Ramsey-Boiteux's rule, which proposes charging higher prices to groups with less elastic demand.
...
::: {#exr-PriceDisc}
Suppose that a monopolist with a constant marginal cost of 4 can identify two markets:
$$
\begin{aligned}
q_1 &= 100 - 2p_1 \\
q_2 &= 120 - 3p_2.
...
1. If the monopolist is allowed to charge different prices, what will be the quantity sold to each group of customers? the prices charged? the consumer surplus?
2. If the monopolist is forced to charge a single price, what will this price be? What is the consumer surplus now? 
3. Compute the demand elasticities associated with the prices and quantities computed in the first question. How can one use the Ramsey-Boiteux's rule to improve market efficiency?
:::