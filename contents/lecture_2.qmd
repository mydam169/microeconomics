...
$$
\begin{aligned}
&\mathcal{L}_L (L^*, K^*, \lambda^*) = 0, \:
\mathcal{L}_K (L^*, K^*, \lambda^*) = 0, \\
% &\quad \quad \quad \text{ with equality for interior solutions }\\
&\lambda^* \geq 0, \: q(L^*, K^*) - m \geq 0, \:  \lambda^*[q(L^*, K^*) - m] = 0.
\end{aligned}
$${#eq-KT_cmp}
...
* Explicit costs (also called accounting costs): Costs that require monetary payments

* Implicit costs (or opportunity costs): Do not require monetary outlays and reflect what one must sacrifice when making a choice

* Example: What is your cost of attending this course?
    * Accounting costs: tuition, transportation, etc.
    * Opportunity cost: the salary you could have earned during the time of study
