...
1. Every firm is maximizing profits: $p=MC(q)$
2. Each firm makes zero economic profits: $p=AC(q)$
    * This means that firms make the same profits in other competitive markets (hence no incentives for other firms to enter or for existing firms to exit)
3. Market demand equal to LR market supply
    * $q^S(p) = q^D(p)$
...
In optimality:
$$
MRS^i_{1,2} = \frac{p_1}{p_2} \iff p_1x_1^i = p2_x2^i.
$${#eq-FOCexm}
...
* Consumers own factors of production, which they can rent out to firms to earn income

* Firms choose the quantity of factors of production to maximize profit, taking as given the prices (of factors and final consumption goods)

* Consumers choose the quantity of consumption goods to maximize utility, taking as given prices (of factors and final consumption goods) 
...
* Consider an economy two consumers with Cobb-Douglas utility function
    ...
    $$ 
    for each consumer $i=A, B$.

* Their endowments:
    $$
    e^A = (k^A, l^A), \quad e^B = (k^B, l^B).
    $${#eq-endowment}
    
* There are two firms in the economy, each producing one of the consumption goods according to a Cobb-Douglas technology:
...
An allocation $(x_1^A, x_1^B, x_2^A, x_2^B, K_1, L_1, K_2, L_2)$ and a price vector $p=(p_1, p_2, w, r)$ constitute a competive equilibrium (or Walrasian equilibrium) the production economy if:
...
* Consumer $i$ solves:
    $$
    \max_{x^i} u(x^i) \text{    s.t.   } px^i \leq wl^i + rk^i.
    $${#eq-UMP}
...
* @eq-MRS_i implies:
    ...
    which together with the binding BC provide the optimal consumption bundle for each consumer as function of factor prices and endowments.
...
* Observe that @eq-MRS_i and @eq-prices_PMP together say that at an optimum, the economy transforms one good for the other at the rate consumers are willing to substitute one for the other.
...
* Having computed factor demands, we can substitute these values into @eq-prices_UMP to determine equilibrium price ratio $p_1/p_2$

* We can then normalize $p_2\equiv 1,$ which implies
...
* Factor prices can then be determined from @eq-PMPj_FOC

* We can then compute $x^A, x^B$ by substituting factor and consumption prices into @eq-demand_conso

# The social planner's problem in a pure exchange economy

In a centralized economy, the benevolent social planner allocates resources in such a way that maximizes a social welfare function (weighted sum of utilities) subject to some feasibility constraints. A solution to the social planner's problem is Pareto efficient. Mathematically, the social planner solves:
$$
\begin{aligned}
...
* In this particular example consumers have identical preferences. Hence:
...
* Hence in a pure exchange economy the PEA is proportional to the social weight of the individual. If the social weights are equal, the aggregate endowments will be divided equally among the consumers. The results will be different if:

    * Social weights are different
    * Preferences are different
    
## The social planner's problem in a production economy

* According to the FWT the CEA in the production economy also Pareto efficient
    
* Let us show that the CEA is also a PEA, i.e., that the FWT is verified in this production economy.
...
* The first constraint is the feasibility constraint for each consumption good

* The last two constraints are feasibility constraints for the factors of production
...
* These conditions being satisfied by the WEA, implying that the WEA is also PE.
:::
# Exercises
