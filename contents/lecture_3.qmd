...
Given $p$, the market price of the good, the firm's PMP is:
$$
\max_{q\geq 0} \pi = R(q) - C(q) = pq - C(q), 
$$
where $C(q)$ is the total cost function derived from the CMP introduced previously and $R(q)$ denotes the total revenue. 
:::
...
* Hence in the LR equilibrium, the price will be about $\$3.94$ per unit. At this price, about $92$ units will be produced by about $41$ firms.
...
3. The equilibrium price $p$ clears all markets. This means that the aggregate demand is equal to aggregate supply for each good:[^2]
    $$ 
    q_j^D(p_j) = q_j^S(p_j),\quad \forall j=1,\dots,L. 
    $$
:::

[^2]: In the case of a pure exchange economy, the aggregate supply of each good is just equal to its total endowment.

Recall that for each consumer that has an interior solultion to their UMP, the marginal utility per dollar is equalized across the goods. Mathematically:
...
In economics, the notion of efficiency of interest is Pareto efficiency. We say that an allocation is __Pareto efficient__ if there exists no other allocation that is strictly better off for someone without being worse off for someone else. In other words, an efficient allocation is one that cannot be improved for someone without hurting someone else: any change to it penalizes at least an individual. In a market with no market failures, it can be shown that under mild conditions, a CEA is Pareto efficient. This is the first welfare theorem (FWT). Graphically, the Pareto efficient allocation lies at the tangency of two ICs in the Edgeworth box. This means that such allocations are not unique, and in fact the locus of all __Pareto efficient allocations (PEA)__ forms the so-called __contract curve__. How come we found only one of them in the previous example? This is because which Pareto efficient allocation can be achieved by market forces alone depends on the initial endowment. If we wish to alter the outcome, we need start differently. This is the idea behind the second welfare theorem (SWT).

Another natural question that one might ask is why would an economy prefer a different Pareto efficient allocation? Why bother redistributing resources to incentivise the market to achieve a different allocation at all? It is because on top of efficiency, societies also care about equality. If one starts out with abundant resources, then market forces alone tend to lead to an efficient allocation that gives more to those with more initial resources. In other words, competitive market forces tend to reinforce existing inequality.
...
::: {#exr-NoExcessDemand} 
Show that if the market clearing condition in @def-GE is satisfied for $L-1$ markets then it is also satisfied for the remaining one. This follows from the Law of Walras. Thanks to this result, we have one fewer market to consider.
:::
...
::: {#exr-SWT}
Show that in @exm-EQM-SWT, the general way to achieve the PEA as a CEA is by redistributing initial resources such that:
$$
\begin{aligned}
e_1^A &= z \in [0, 200], \\
e_2^A &= 400 - 2z, \\
